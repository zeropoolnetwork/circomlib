...
const {multiHash} = require("../src/mimc7.js");
...
describe("Merkle proof test test", () => {
    ...
    it("Should create a merkle proof circuit", async () => {
        ...
        for(let i=0; i<n; i++){
          ...
          root = _path[i]==0n ? multiHash([root, sibling[i]]) : multiHash([sibling[i], root]);
        }
        
        const cirDef = await compiler(path.join(__dirname, "circuits", "merkleproofmimc_test.circom"));
